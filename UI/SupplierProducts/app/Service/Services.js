...
var ConsumerService = (function () {
    function ConsumerService(_http) {
        this._http = _http;
        this._url = "http://192.168.1.128:8090/mvp-service-webapp/getConsumer?consumerId=1 ";
    }
    ConsumerService.prototype.getConsumers = function () {
        return this._http.get(this._url)
            .map(function (response) { return response.json(); })
            .catch(this._errorHandler);
    };
    ConsumerService.prototype._errorHandler = function (error) {
        console.error(error);
